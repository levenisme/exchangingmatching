...
func get_compare_info(db *sql.DB, sym string, amount string, limit string, is_buy bool){
	query_buy := fmt.Sprintf("select order_id, open, price from order_info where ((type == %s ) and (limit <= %s) and (open < 0 )) order by limit ASC, order_id ASC;", string(EXECUTING), limit)

	query_sell := fmt.Sprintf("select order_id, open, price from order_info where ((type == %s ) and (limit >= %s) and (open > 0 )) order by limit DESC, order_id ASC;", string(EXECUTING), limit)
